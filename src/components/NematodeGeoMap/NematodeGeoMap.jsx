// src/components/NematodeGeoMap/NematodeGeoMap.jsx
import React, {
  useEffect,
  useState,
  useRef,
...
const HistoricalMap = () => {
  ...
  // [CHANGED] Wrap MapContainer so overlays sit outside
  return (
...
const NematodeGeoMap = () => {
  ...
  // [ADDED] ref + handler for snapshot downloads
  ...
  useEffect(() => {
    const fetchNewMapData = async () => {
      ...
      try {
        ...
        setNewMapSelectedNematodeGroups([]); // start with none selected
      } catch (e) {
                ...
                <div className="mt-4">
                  <ul className="space-y-2 text-sm text-slate-700">
                    <li className="flex items-center gap-3">
                      ...
                      <span><strong>Confirmed</strong> – nematodes have been detected in these areas.</span>
                    </li>
                    <li className="flex items-center gap-3">
                      <span className="inline-block h-4 w-4 shrink-0 rounded-full bg-[#e5e7eb] ring-1 ring-[#e5e7eb]/60" />
                      <span><strong>Unconfirmed</strong> – nematodes may occur, but not yet verified.​</span>
                    </li>
                  </ul>
                </div>
              ...
              <div className="bg-white rounded-2xl shadow p-4 sticky top-[84px] max-h-[calc(100vh-120px)] flex flex-col">
                      ...
                      {filteredGroups.map((group) => {
                        const formatName = (name) => {
                          ...
                          if (parts.includes("spp.") || parts.includes("sp.")) {
                            return (<><i>{parts[0]}</i> {parts.slice(1).join(" ")}</>);
                          }
                          if (parts.length >= 2) {
                            return (<><i>{parts[0]} {parts[1]}</i> {parts.slice(2).join(" ")}</>);
                          }
                          return name;
                        };
                        ...
                        return (
                          ...
                          >
                            <div className="flex items-center gap-2 min-w-0">
                              ...
                              />
                              <span className="text-sm truncate">{formatName(group)}</span>
                            </div>
                            <input
                              type="checkbox"
                              value={group}
                ...
                {!showHistoricalMap && (
                  <div className="mt-3 text-xs text-slate-600">
                    Markers are colored by Common name. Multiple points at the same location are gently offset so each remains clickable.
                  </div>
                )}
              </div>
            )}
